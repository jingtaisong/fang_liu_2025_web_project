...
class upimg_class{
    public $uploadFolder = './upload'; // Image storage directory
    public $thumbFolder = './upload/thumb'; // Thumbnail storage directory
    ...
    // Description: Initialize, create storage directories
    ...
    // Description: Upload image, parameter is the name attribute of <input />; Returns relative URL of image on success, returns FALSE and error message (in $this->error) on failure
    // bool/sting upload(string $html_tags_input_attrib_name);
    function upload($inputName){ // Upload operation, parameter is input tag's name attribute
        if ($this->error){ // If there's an error, return directly (e.g. _mkdir)
            return FALSE;
        }
        ...
        if($_FILES[$inputName]["name"]){
            ...
            if (is_uploaded_file($isUpFile)){
                ...
                if (1 == $uploadError){ // File size exceeds upload_max_filesize in php.ini
                    ...
                    $this->error = 'Upload interrupted, please retry';
                    return FALSE;
    ...
    // Description: Get image information, parameter is the uploaded temporary file, returns array on success, returns FALSE and error message on failure
    ...
    private function _getinfo($img){
        ...
        if ($fileType == 'png' || $fileType == 'gif' || $fileType == 'jpg'){
            ...
            return $imgInfo;
        }else{ // Non-image file information
            $this->error = 'Invalid image type';
            return FALSE;
        }
    } // end _getinfo

    // Description: Generate thumbnail, proportional scaling or stretching
    ...
    function thumb($img, $thumbWidth = 400, $thumbHeight = 300, $thumbTail = '_thumb'){
        $filename = $img; // Keep a name for the new thumbnail
        $imgInfo = $this->_getinfo($img);
        if (FALSE == $imgInfo){
            return FALSE;
        }
        $imgType = $imgInfo['type'];
        switch ($imgType){ // Create an image and give it an extension
            case 'gif' :
            ...
            case "jpg" :
                ...
                $extName = 'jpg';
                break;
            default : // If the type is incorrect, generate a blank image
                $img = imagecreate($thumbWidth,$thumbHeight);
                imagecolorallocate($img,0x00,0x00,0x00);
                $extName = 'jpg';
        }
        // The size of the scaled image (stretch if small, scale down if large)
        $imgWidth = $imgInfo['width'];
        $imgHeight = $imgInfo['height'];
        if ($imgHeight > $imgWidth){ // Portrait
            $newHeight = $thumbHeight;
            $newWidth = ceil($imgWidth / ($imgHeight / $thumbHeight ));
        }else if($imgHeight < $imgWidth){ // Landscape
            $newHeight = ceil($imgHeight / ($imgWidth / $thumbWidth ));
            $newWidth = $thumbWidth;
        }else if($imgHeight == $imgWidth){ // Square
            $newHeight = $thumbWidth;
            $newWidth = $thumbWidth;
        } 
// The following is for proportional scaling
        ...
        if(!$sampled ){
            ...
            @unlink($this->uploadFolder . '/' . $filename); // Delete the uploaded image
            return FALSE;
        }
    ...
    // Description: Create directory for storing images
    private function _mkdir(){ // Create directories for image upload and thumbnails
        if(!is_dir($this->uploadFolder)){
            $mkdirResutlt = @mkdir($this->uploadFolder);
            if (!$mkdirResutlt){
                $this->error = 'Failed to create image upload directory';
                return FALSE;
            }
        }
        if(!is_dir($this->thumbFolder) && TRUE == $this->autoThumb){
            ...
            if (!$mkdirResutlt){
                $this->error = 'Failed to create thumbnail directory';
                return FALSE;
            }
        }
    }
